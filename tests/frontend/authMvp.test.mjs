...
export function runFrontendTests() {
  ...
  assert.equal(registrationResult.session.authenticated, true, "после регистрации пользователь должен автоматически войти в систему");
  ...
  assert.equal(duplicateRegistration.errors.email, "Пользователь с таким email уже существует", "должна отображаться ошибка о существующем email");
  ...
  assert.equal(shortPasswordErrors.password, "Пароль должен быть не короче 6 символов", "должна отображаться ошибка при коротком пароле");
  ...
  assert.equal(mismatchErrors.confirmPassword, "Пароль и подтверждение пароля не совпадают", "должна отображаться ошибка при несовпадении паролей");
  ...
  const authenticatedSession = {
    authenticated: true,
    user: { id: "user-1", email: "anna@example.com", name: "Анна" }
  };
  assert.equal(canAccessProtectedPage(authenticatedSession), true, "авторизованный пользователь должен иметь доступ к защищённой странице");

  const loggedOutSession = logoutUser(authenticatedSession);
  assert.equal(loggedOutSession.authenticated, false, "после выхода сессия должна завершаться");
  assert.deepEqual(
    resolveProtectedPageAccess(loggedOutSession, "/profile"),
