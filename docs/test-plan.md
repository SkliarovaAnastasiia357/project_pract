...
## Летняя практика: проверка подбора команды

### Автоматические сценарии

- кандидат с навыками `React, TypeScript` и ролью `Frontend developer` получает 70% и занимает первое место;
- совпадение только по роли даёт 40%;
- регистр, дубли и замена `ё/е` не влияют на результат;
- пустые требования проекта дают 0%;
- равные результаты сортируются по имени;
- поиск и фильтр не изменяют исходный массив;
- список маршрутов содержит `/projects/:id/candidates`;
- смена SPA-маршрута сбрасывает прокрутку к началу страницы.

### Browser smoke в mock-режиме

1. Зарегистрировать владельца и заполнить демо-данные.
2. Перейти из карточки собственного проекта по действию `Подобрать команду`.
3. Проверить порядок и проценты трёх кандидатов: 70%, 40%, 40%.
4. Выполнить поиск `React` и включить `Только с совпадениями`.
5. Обновить защищённый маршрут напрямую и подтвердить восстановление сессии.
6. Проверить desktop и viewport 390×844; `scrollWidth` должен совпадать с `innerWidth`.
...
Документный артефакт дополнительно проверяется постраничным PNG-рендером, аудитом доступности, изображений, заголовков, секций и точной геометрии таблиц.
