...
Финальный MVP Teamnova продолжает развиваться командой ChamBucket после проектного практикума; новый или сторонний проект для летней практики не создавался. Итерация летней практики добавляет владельцу проекта проактивный подбор команды по стеку и ролям без изменения backend API.
...
- На карточках собственных проектов доступно действие `Подобрать команду`; защищённый маршрут `/projects/:id/candidates` показывает ранжированные профили, объяснения, поиск, фильтр и email-контакт.
- Подготовлен 12-страничный отчёт по летней проектной практике с матрицей ответственности и четырьмя скриншотами реально запущенного приложения.
...
- 2026-07-10 gate итерации подбора команды:
  - `npm run test:frontend` прошёл, включая scorer, фильтры, маршрут и regression сброса прокрутки.
  - `npm run test:backend` прошёл полностью: 19 test files, 81 passed, 0 skipped.
  - `npm run build`, `npm run build:backend` и `npm run lint` прошли.
  - Desktop browser smoke подтвердил переход из проекта, рейтинг 70/40/40, объяснения, поиск, фильтр и прямой refresh защищённого маршрута.
  - Mobile smoke на 390×844 подтвердил `scrollWidth = innerWidth = 390` и отсутствие горизонтального переполнения.
  - DOCX отрендерен в 12 PNG-страниц; проверены все страницы, 4 inline-изображения, доступность без замечаний и точная геометрия трёх таблиц.
...
5. В карточке собственного проекта нажать `Подобрать команду` и показать рейтинг 70/40/40, объяснения и фильтр.
6. Через `/search` найти проект и показать объяснение match score участника.
7. Открыть `/requests`, принять кандидата и показать его в команде проекта.
8. Вернуться на `/home`, проверить live dashboard и состав команды.
9. Очистить демо-данные кнопкой `Очистить` или оставить их на автоочистку по TTL.
10. Обновить protected route и подтвердить, что mock-сессия восстановилась.
