...
- Главная страница и доска задач обновлены под финальный gate: automated checks, ручной MVP-cycle, готовая презентация, документация и внешняя проверка хостинга.
...
- Проверен desktop/mobile layout: карточки и доска задач не обрезаются, формы и статусы читаемы.
